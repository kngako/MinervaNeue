...
class MinervaTemplate extends BaseTemplate {
	...
	public function getPageViewActions() {
		return $this->isFallbackEditor() ? [] : $this->data['page_actions'];
	}
	...
	protected function getPageActionsHtml( $data ) {
		...
		if ( $actions ) {
			foreach ( $actions as $key => $val ) {
				...
				if($key == 'edit') {
					// Translate functions should be handled here...
					$data['content_actions']['edit']['class'] .= ' mw-ui-icon mw-ui-icon-element '; 
					$html .= $this->makeListItem( $key, $data['content_actions']['edit'] );
				} else {
					$html .= $this->makeListItem( $key, $val );
				}
			}
			// foreach ($data['content_actions'] as $key => $item ) {
			// 	$html .= '<p class="kgothatso">' . $key . '</p>';
			// }
